...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:z="https://www.zerocracy.com" version="2.0" exclude-result-prefixes="xs z">
  ...
  <xsl:function name="z:format-signed">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$formatted = '-0.0' or $formatted = '-0.00'">
        <xsl:value-of select="substring($formatted, 2)"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="$formatted"/>
      </xsl:otherwise>
    ...
    <html lang="en">
      ...
      <head>
        ...
        <xsl:variable name="description">
          <xsl:variable name="facts" select="$fb/f[z:when(when) &gt; (xs:dateTime($today) - xs:dayTimeDuration('P256D')) and z:when(when) &lt;= xs:dateTime($today) and award and is_human = 1]"/>
