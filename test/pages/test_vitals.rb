...
class TestVitals < Minitest::Test
  ...
  def test_names_the_plugin_on_an_adless_page
    text = Nokogiri::HTML.parse(generate_vitals_html(adless: 'true')).text.gsub(/\s+/, ' ')
    assert_includes(text, 'generated by the pages-action plugin', text)
  end
